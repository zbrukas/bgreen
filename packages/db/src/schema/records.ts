...
export const records = pgTable(
  ...
  (t) => ({
    orgCreatedIdx: index("records_org_created_idx").on(t.organizationId, desc(t.createdAt)),
    templateIdx: index("records_template_idx").on(t.templateId),
    submitterIdx: index("records_submitter_idx")
      .on(t.submittedByUserId)
      .where(sql`${t.submittedByUserId} IS NOT NULL`),
  }),
);
