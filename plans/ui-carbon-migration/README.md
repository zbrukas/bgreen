...
> **Status: COMPLETE** (2026-05-25). All shadcn primitives migrated to Carbon and `apps/{web,cs}/src/components/ui/` directories deleted. Both apps run Carbon-only chrome AND form bodies. `packages/ui` houses shared `PageHeader` / `EmptyState` / `StatCard`. IBM Plex self-hosted via `@ibm/plex-*`. `lucide-react` and `class-variance-authority` removed.
