...
- **V6.4 (shipped):** `EconomicProfileRepository` (Drizzle upsert with money serialized as `numeric` strings, decoded to JS numbers at the boundary). `IesUploadService` (25 MB cap, MIME check, log-row-before-S3-write ordering). `IesExtractionService.confirm` / `.cancel` / `.getStatus` (S3 cleanup on terminal transitions). `EconomicProfileService.manualEntry` for the fallback path. Six HTTP endpoints under `/economic-profile`: `POST /ies` (multipart), `GET /ies/:id` (status poll), `POST /ies/:id/confirm`, `POST /ies/:id/cancel`, `POST /manual`, `GET /` (list / by-year). Mutating routes gated by `org_admin` or `org_user_write`; read by any membership. Smoke-tested via live boot — Inngest reports `function_count: 1`, all routes return 401 unauthenticated.
- **V6.5 (shipped):** `apps/web` IES flow. `@tanstack/react-query` ^5.66 mounted via a `Providers` client component (staleTime 30s, no window-focus refetch — we poll deliberately). Server actions in `lib/economic-profile-actions.ts` proxy apps/api with the user's WorkOS session (token stays server-side). Four pages: `/economic-profile` (list with pt-PT €-formatted table + empty-state CTA), `/economic-profile/ies/new` (two-step consent → drag-drop with client-side size/MIME guard, `useMutation` upload), `/economic-profile/ies/[logId]` (status polling via `useQuery` with `refetchInterval` 2s stopping on terminal status; inline-editable result form with confidence badges + validator warnings; confirm/cancel mutations invalidate per-log + list caches), `/economic-profile/manual` (fallback form). Smoke-tested live — all three new routes return 307 unauthenticated, no compile errors.
- **V6.6 (shipped):** `PostHogTelemetry` facade (Active / NoOp variants — boot doesn't break when `POSTHOG_API_KEY` is missing; EU host `eu.i.posthog.com` by default; SIGTERM-flush). `createAiToolCallPostHogObserver` captures `ai.tool_call` event with toolName, outcome, errorKind, latency, tokens, and approximate USD cost from a hard-coded price table (Sonnet 4.6 $3/$15/M, Opus 4.7 $5/$25/M, Haiku 4.5 $1/$5/M, cache writes 1.25×, reads 0.1×). `composeObservers([...])` helper in `@bgreen/ai` fans out a single observer hook to multiple sinks with parallel + error-isolated execution (synchronous throws caught via async-wrapper IIFE). Audit + PostHog now compose into one observer in `services.ts`. Home page gains "Carregue o seu IES para desbloquear recomendações" CTA when the org has no profiles, or a compact summary card showing year + employees + turnover when it does. Header adds "Perfil económico" nav link. 5 new tests cover PostHog observer success/error/system-actor paths + composeObservers fanout + error isolation.
