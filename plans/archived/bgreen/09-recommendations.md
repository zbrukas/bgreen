...
- **V9.3 (shipped):** UI — Gerar button, polling, result cards with feedback chips, history, preliminary mode. `/recommendations` lists generation history (table with date, completeness, status badge, item count, "úteis" count) + always-on `Gerar recomendações` button that fires `startRecommendations` and routes to the detail page. `/recommendations/[id]` polls every 2s while the run is pending/running (`tanstack-query` `refetchInterval` stops on terminal), then renders one `RecommendationCard` per item with impact/effort/horizon badges + expandable rationale + 5-chip feedback row (`útil`, `já implementada`, `não aplicável`, `irrelevante`, `incorreta`); clicking a chip upserts via `submitRecommendationFeedback` and patches the local cache. `AiBanner` ("Recomendações geradas por IA — valide com o seu consultor.") shows on both screens. Header nav gains a `Recomendações` link. Dashboard `RecommendationsCta` switches between preliminary (INCOMPLETE-mode) and standard copy. All monorepo typecheck targets green; `/recommendations` + `/recommendations/[id]` build successfully (Next.js dynamic routes); boot smoke confirms `/recommendations` requires auth (401) and the web route redirects (307) without a session.
