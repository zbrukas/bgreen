...
> **Status:** Shipped (V11.1 + V11.2 + V11.3 + V11.4).
...
- **V11.4 (shipped):** UI — reports list, Gerar flow, polling page, branding settings. `/reports` server-renders the `getReportsHistory` table with status badges + per-row "Abrir" link + an admin-only "Gerar relatório PDF" CTA. `/reports/new` is the admin-only generation form: radio template picker (ghg-inventory / esrs-e1 / custom), period selector with `Ano corrente` / `Ano anterior` / `Trimestre corrente` shortcuts + manual ISO date inputs, optional custom title (custom template only). For ESRS E1 the form fetches V10's coverage matrix via `getCoverageMatrix("esrs")` and surfaces a pt-PT warning ("Cobertura incompleta: N em falta, M parcial") before submit — user can still proceed. Submit → POST /reports → 202 → router pushes to `/reports/[id]`. `/reports/[id]` polls every 3s until terminal via tanstack-query, renders the status card with progress block for pending/running, error message for failed, info banner for cancelled. On ready: surfaces AI commentary sections, integrity hash (SHA-256), and a "Descarregar PDF" button that calls a server action which follows apps/api's 302 manually and returns the presigned S3 URL for `window.open`. `AiBanner` ("Comentário gerado por IA…") on all three screens. Branding side: `PATCH /organizations/:id/branding` (org-admin only, validates `#rrggbb` color) + `POST /organizations/:id/branding/logo-upload-url` (presigned S3 PUT, 5 extensions: png/svg/jpg/jpeg/webp). `/organizations/[orgId]/branding` web settings page seeded from `fetchActiveOrganization`: two-step upload (PUT direct to S3 → PATCH with key) + color picker with hex input + live swatch. `OrganizationService.updateBranding` writes `organization.branding_updated` audit. Header gains "Relatórios" + "Identidade" links. Web build adds the four new routes (`/reports`, `/reports/new`, `/reports/[id]`, `/organizations/[orgId]/branding`). monorepo: 18 typecheck targets clean; 174 tests in apps/api. Boot smoke: api routes 401 without auth as expected.

## V1 complete

V11 closes V1 of bGreen. The end-to-end product surface is live:
signup → org profile → records → coverage → recommendations → PDF
reports. v1.5 backlog (per parent PRD): eval suites, curated
recommendation library, full ESRS catalogue, automated benchmark
refresh, monitoring dashboards, feature flags, rate limits, cost
budgets, closed-beta rollout.
